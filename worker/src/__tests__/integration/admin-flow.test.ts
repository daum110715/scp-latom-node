...
describe('Admin Flow Integration', () => {
  ...
  describe('Entry Management', () => {
    ...
    it('updates entry name and object class', async () => {
      ...
      const detailRes = await app.request('/api/admin/entries/1', {
        method: 'GET',
        headers: { Authorization: `Bearer ${token}` },
      }, env)
  ...
  describe('Proposal Management', () => {
    ...
    async function seedProposal(env: any, token: string) {
    ...
    it('deletes a proposal and cascades votes', async () => {
      ...
      const detailRes = await app.request('/api/admin/proposals/1', {
        method: 'GET',
        headers: { Authorization: `Bearer ${token}` },
      }, env)
      expect(detailRes.status).toBe(404)
  ...
  describe('Tag Management', () => {
    ...
    it('deletes a category', async () => {
      ...
      const res = await app.request('/api/admin/tags/categories/to-delete', {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      }, env)
      expect(res.status).toBe(200)
    ...
    it('creates a tag', async () => {
      ...
      const res = await app.request('/api/admin/tags', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ id: 'new-tag', category_id: 'object-class', name: '新标签', name_zh: '新标签' }),
      }, env)
      const body = await parseJson(res)
      expect(res.status).toBe(201)
    ...
    it('rejects duplicate tag ID', async () => {
      ...
      const res = await app.request('/api/admin/tags', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ id: 'safe', category_id: 'object-class', name: 'Dup', name_zh: 'Dup' }),
      }, env)
      expect(res.status).toBe(409)
    })
    ...
    it('deletes a tag', async () => {
      ...
      const res = await app.request('/api/admin/tags/to-delete', {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      }, env)
      expect(res.status).toBe(200)
    ...
    it('removes a tag from an entry', async () => {
      ...
      // First assign
      await app.request('/api/admin/tags/entry/173', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  describe('Log Management', () => {
    ...
    it('searches logs by message', async () => {
      ...
      const res = await app.request('/api/admin/logs?q=Database', {
        method: 'GET',
        headers: { Authorization: `Bearer ${token}` },
      }, env)
      const body = await parseJson(res)
