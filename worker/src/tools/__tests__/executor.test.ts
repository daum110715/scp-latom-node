      ...
      const stmt = {
        ...
        first: vi.fn(async () => {
          ...
          if (sql.includes('WHERE scp_number = ?')) {
            ...
            return rows.find(
              (r) => r.scp_number === scp_number && r.language === language
            ) ?? null
          }
          return null
        }),
        all: vi.fn(async () => {
          ...
          const limitParam = sql.includes('LIMIT') ? stmt._params[stmt._params.length - 1] : undefined
          ...
          if (sql.includes('LIKE')) {
            ...
            filtered = rows.filter(
              (r) => (r.name as string).toLowerCase().includes(query)
            )
          } else if (sql.includes('object_class = ?')) {
            const objectClass = stmt._params.find((p) => typeof p === 'string' && !p.includes('%') && p.length > 2)
...
const mockEntries = [
  { scp_number: 173, name: 'The Sculpture', object_class: 'Euclid', language: 'en', content: 'SCP-173 is a concrete sculpture...' },
  { scp_number: 682, name: 'Hard-to-Destroy Reptile', object_class: 'Keter', language: 'en', content: 'SCP-682 is a large, vaguely...' },
  { scp_number: 999, name: 'Tickle Monster', object_class: 'Safe', language: 'en', content: 'SCP-999 is a large, amorphous...' },
  { scp_number: 173, name: '雕像', object_class: 'Euclid', language: 'cn', content: 'SCP-173是一个混凝土雕塑...' },
]
...
describe('executeTool', () => {
  ...
  describe('get_scp_entry', () => {
    ...
    it('supports Chinese language', async () => {
      const result = await executeTool(db as any, 'get_scp_entry', { scp_number: 173, language: 'cn' })
      const parsed = JSON.parse(result)
  ...
  describe('list_scp_entries_by_class', () => {
    it('returns entries filtered by class', async () => {
      const result = await executeTool(db as any, 'list_scp_entries_by_class', { object_class: 'Keter' })
      const parsed = JSON.parse(result)
    ...
    it('returns empty for class with no entries', async () => {
      db = createMockDb([])
      const result = await executeTool(db as any, 'list_scp_entries_by_class', { object_class: 'Thaumiel' })
      const parsed = JSON.parse(result)
    ...
    it('respects limit parameter', async () => {
      const result = await executeTool(db as any, 'list_scp_entries_by_class', { object_class: 'Euclid', limit: 1 })
      const parsed = JSON.parse(result)
