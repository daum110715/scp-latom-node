    ...
    prepare: (sql: string) => {
      const stmt = {
        ...
        first: async (): Promise<any> => {
          if (
            sql.includes('COUNT(*)') &&
            sql.includes('proposals WHERE user_id = ? AND created_at >= date')
          ) {
          ...
          if (
            sql.includes('SELECT vote FROM proposal_votes WHERE proposal_id = ? AND user_id = ?')
          ) {
            return userVote
          }
    ...
    it('creates a proposal', async () => {
      ...
      const res = await app.request(
        '/api/proposals',
        {
          ...
          body: JSON.stringify({
            title: 'New Proposal',
            content: 'This is a valid proposal content.',
            category: 'general',
          }),
        },
        env,
    ...
    it('rejects invalid category', async () => {
      ...
      const res = await app.request(
        '/api/proposals',
        {
          ...
          body: JSON.stringify({
            title: 'Valid Title',
            content: 'This is valid content for the proposal.',
            category: 'invalid',
          }),
        },
        env,
  ...
  describe('POST /api/proposals/:id/vote', () => {
    ...
    it('records a vote', async () => {
      ...
      const env = createEnv({
        ...
        existingVote: null,
        voteCounts: { vfor: 1, against: 0, abstain: 0 },
      })
      const res = await app.request(
        '/api/proposals/1/vote',
        {
