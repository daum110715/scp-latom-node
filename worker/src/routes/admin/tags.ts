...
tags.get('/stats', async (c) => {
  ...
  const totalEntryTags = await c.env.DB.prepare(
...
tags.post('/categories', async (c) => {
  ...
  const body = await c.req.json<{ id?: string; name?: string; name_en?: string; description?: string; sort_order?: number }>()
  ...
  ).bind(id, name, nameEn, description, sortOrder).run()
...
tags.put('/categories/:id', async (c) => {
  ...
  const body = await c.req.json<{ name?: string; name_en?: string; description?: string; sort_order?: number }>()
  ...
  if (body.name_en !== undefined) { sets.push('name_en = ?'); params.push(body.name_en.trim()) }
  ...
  params.push(id)
  await c.env.DB.prepare(
    `UPDATE tag_categories SET ${sets.join(', ')} WHERE id = ?`
  ).bind(...params).run()
...
tags.delete('/categories/:id', async (c) => {
  ...
  return c.json({ success: true, message: `Category deleted along with ${tagCount?.count ?? 0} tags` })
})
...
tags.post('/', async (c) => {
  ...
  await c.env.DB.prepare(
    `INSERT INTO tags (id, category_id, name, name_zh, description, ai_keywords, sort_order)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).bind(id, categoryId, name, nameZh, description, JSON.stringify(aiKeywords), sortOrder).run()
...
tags.put('/:id', async (c) => {
  ...
  if (body.category_id !== undefined) { sets.push('category_id = ?'); params.push(body.category_id.trim()) }
  if (body.name !== undefined) { sets.push('name = ?'); params.push(body.name.trim()) }
  if (body.name_zh !== undefined) { sets.push('name_zh = ?'); params.push(body.name_zh.trim()) }
  if (body.description !== undefined) { sets.push('description = ?'); params.push(body.description.trim()) }
  if (body.ai_keywords !== undefined) { sets.push('ai_keywords = ?'); params.push(JSON.stringify(body.ai_keywords)) }
  if (body.sort_order !== undefined) { sets.push('sort_order = ?'); params.push(body.sort_order) }
...
tags.post('/entry/:scpNumber', async (c) => {
  ...
  for (const tagId of tagIds) {
    ...
    ).bind(scpNumber, language, tagId).run()
