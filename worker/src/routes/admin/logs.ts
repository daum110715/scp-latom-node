...
logs.get('/', async (c) => {
  ...
  const countRow = await c.env.DB.prepare(`SELECT COUNT(*) as total FROM system_logs ${where}`)
    .bind(...params)
    .first<{ total: number }>()
...
logs.get('/stats', async (c) => {
  const [byLevel, bySource, byCategory, errorRate] = await Promise.all([
    c.env.DB.prepare('SELECT level, COUNT(*) as count FROM system_logs GROUP BY level').all<{
      level: string
      count: number
    }>(),
    c.env.DB.prepare('SELECT source, COUNT(*) as count FROM system_logs GROUP BY source').all<{
      source: string
      count: number
    }>(),
    c.env.DB.prepare(
...
logs.get('/:id', async (c) => {
  ...
  const log = await c.env.DB.prepare('SELECT * FROM system_logs WHERE id = ?')
    .bind(id)
    .first<AdminLogEntry>()
...
logs.delete('/cleanup', async (c) => {
  ...
  await c.env.DB.prepare('DELETE FROM system_logs WHERE timestamp < ?').bind(cutoff).run()
