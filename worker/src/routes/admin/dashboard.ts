...
dashboard.get('/', async (c) => {
  ...
  const [
    ...
    db
      .prepare('SELECT language, COUNT(*) as count FROM scp_entries GROUP BY language')
      .all<{ language: string; count: number }>(),
    db
      .prepare('SELECT object_class, COUNT(*) as count FROM scp_entries GROUP BY object_class')
      .all<{ object_class: string; count: number }>(),
    db
      .prepare('SELECT status, COUNT(*) as count FROM proposals GROUP BY status')
      .all<{ status: string; count: number }>(),
    db
      .prepare("SELECT COUNT(*) as count FROM users WHERE created_at >= date('now')")
      .first<{ count: number }>(),
    db
      .prepare("SELECT COUNT(*) as count FROM proposals WHERE created_at >= date('now')")
      .first<{ count: number }>(),
    db
      .prepare("SELECT COUNT(*) as count FROM proposal_votes WHERE created_at >= date('now')")
      .first<{ count: number }>(),
    db
      .prepare(
    ...
    db
      .prepare(
        "SELECT COUNT(*) as total, SUM(CASE WHEN level = 'error' THEN 1 ELSE 0 END) as errors FROM system_logs WHERE timestamp >= datetime('now', '-7 days')",
      )
