  ...
  <div class="user-detail">
    ...
    <div v-else-if="store.currentUser" class="user-info">
      <div class="admin-card user-card">
        ...
        <!-- Edit Controls -->
        <div class="edit-controls" v-if="store.currentUser.role !== 'admin'">
          <div class="edit-section">
            <span class="edit-label">Role:</span>
            <template v-if="showRoleEdit">
              <select class="select" v-model="newRole">
            ...
            <template v-else>
              <button class="btn btn-ghost btn-sm" @click="newRole = store.currentUser!.role; showRoleEdit = true">Edit</button>
            </template>
          </div>
          <div class="edit-section">
            <span class="edit-label">Clearance:</span>
            <template v-if="showClearanceEdit">
              <select class="select" v-model.number="newClearance">
              ...
              <button class="btn btn-ghost btn-sm" @click="showClearanceEdit = false">Cancel</button>
            </template>
            <template v-else>
              <button class="btn btn-ghost btn-sm" @click="newClearance = store.currentUser!.clearance; showClearanceEdit = true">Edit</button>
            </template>
          </div>
        </div>
      </div>
      ...
      <div class="tabs">
        ...
        <button class="tab" :class="{ active: activeTab === 'history' }" @click="loadTab('history')">History</button>
        <button class="tab" :class="{ active: activeTab === 'bookmarks' }" @click="loadTab('bookmarks')">Bookmarks</button>
      </div>

      <div v-if="loadingTab" class="loading-state">
        <div class="skeleton" v-for="i in 5" :key="i" style="height: 40px" />
      </div>

      <div v-else-if="activeTab === 'history'" class="table-wrap">
        <table class="data-table" v-if="history.length">
          <thead>
            <tr><th>SCP</th><th>Language</th><th>Name</th><th>Class</th><th>Visited</th></tr>
          </thead>
          <tbody>
            <tr v-for="h in history" :key="h.id">
              ...
              <td>{{ h.name }}</td>
              <td><StatusBadge :variant="h.object_class?.toLowerCase() || 'unknown'" :label="h.object_class" /></td>
      ...
      <div v-else-if="activeTab === 'bookmarks'" class="table-wrap">
        <table class="data-table" v-if="bookmarks.length">
          <thead>
            <tr><th>SCP</th><th>Language</th><th>Name</th><th>Class</th><th>Added</th></tr>
          </thead>
          <tbody>
            <tr v-for="b in bookmarks" :key="b.id">
              ...
              <td>{{ b.name }}</td>
              <td><StatusBadge :variant="b.object_class?.toLowerCase() || 'unknown'" :label="b.object_class" /></td>
...
<style scoped>
.user-detail { max-width: var(--max-content); margin: 0 auto; }
.page-header { margin-bottom: var(--space-lg); }

.user-card { margin-bottom: var(--space-lg); }
