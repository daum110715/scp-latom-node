...
describe('api client', () => {
  ...
  describe('apiGet', () => {
    ...
    it('does not send Authorization header (auth via httpOnly cookie)', async () => {
    ...
    it('returns normalized success result', async () => {
      mockFetch.mockResolvedValueOnce(
        mockResponse({ success: true, user: { id: 1, codename: 'test' } }),
      )

      const result = await apiGet('/auth/me')
      expect(result.ok).toBe(true)
      if (result.ok) {
        expect(result.data).toEqual({ user: { id: 1, codename: 'test' } })
      }
    })
